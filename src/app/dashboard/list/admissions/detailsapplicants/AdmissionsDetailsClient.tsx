...
import type { LocalAdmission } from "@/features/admisiones/muck/db";
...
function DetailPanel({
  ...
  const { timeline, addEntry, updateEntry, removeEntry } = useAdmissionTimeline(
    item.id as any
  );
  ...
  // Enroll the aspirante
  const [enrolling, setEnrolling] = useState(false);
  const handleEnroll = async () => {
    if (!item.id) return;
    if (
      !confirm(
        `¿Confirmas inscribir al aspirante ${item.nombreCompleto ?? item.id}?`
      )
    )
      return;
    setEnrolling(true);
    try {
      ...
      alert(
        "No se pudo crear la inscripción en el servidor; se marcará localmente."
      );
    }
    try {
      await admisionesService.updateAdmission(Number(item.id), {
        ...item,
        estatus: "Inscrito",
      });
    } catch {}
      ...
      timeline.forEach((t) => {
        if (t.reminderAt) {
          ...
          if (!isNaN(ts) && ts <= now) {
            ...
            alert(
              `Recordatorio: ${
                t.summary ?? "Contacto programado"
              } para ${t.assignedTo ?? "responsable no asignado"}`
            );
          }
        }
      });
      ...
      <div className="space-y-4">
        ...
        {showContactForm && (
          ...
          >
            ...
            <div>
              <Label>
                Resumen breve de la conversación (se guarda como nota)
              </Label>
              <Textarea
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setContact((s) => ({ ...s, assignedTo: e.target.value }))
                  }
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setContact((s) => ({ ...s, reminderAt: e.target.value }))
                  }
                />
              </div>
            </div>
...
/* ---------- Página cliente: usa useSearchParams / useRouter ---------- */
export default function AdmissionsDetailsClient() {
  const router = useRouter();
  ...
  useEffect(() => {
    ...
    const found = (admissions as AdmisionItem[]).find(
      (a) =>
        String(a.idAspirante ?? a.personaId ?? "") === String(selectedId)
    );
    if (!found) {
      setSelected(null);
      return;
